...
const getAlltime = async (msg) => {
  ...
  bot.sendMessage(
    chatId,
    user.language == "uz"
      ? "<b>Ish grafikni</b> tanlang"
      : "Выберите <b>график работы</b>",
    {
      parse_mode: "HTML",
      reply_markup: {
  ...
  if (jobTime.includes(text)) {
    ...
    if (application) {
      ...
      bot.sendMessage(
        chatId,
        user.language == "uz"
          ? `<b>1-dam olish kunini </b> tanlang`
          : "Выберите <b>1-й выходной день</b>",
        {
          parse_mode: "HTML",
          reply_markup: {
    ...
    if (application) {
      ...
      bot.sendMessage(
        chatId,
        user.language == "uz"
          ? `<b>Supervayzer</b> tanlang`
          : "Выберите <b>Супервайзер</b>",
        {
          parse_mode: "HTML",
          reply_markup: {
            keyboard: [
...
const addSupervazer = async (msg) => {
  ...
  if (Supervayzers.includes(text)) {
    await User.findByIdAndUpdate(
      user._id,
      { ...user, action: "application_comment" },
      { new: true }
    );
    ...
    bot.sendMessage(
      chatId,
      user.language == "uz"
        ? `<b>So‘rov sababini</b> yozing`
        : "Напишите <b>причину запроса</b>",
      {
        parse_mode: "HTML",
        reply_markup: {
          remove_keyboard: true,
...
const addDayOffFirst = async (msg) => {
  ...
  if (DaysUz.includes(text) || DaysRu.includes(text)) {
    ...
    filteredArr.map((e, i) =>
      (i + 1) % 2 != 0
        ? sortDaysArr.push([filteredArr[i], filteredArr[i + 1]])
        : null
    );

    await User.findByIdAndUpdate(
      user._id,
      { ...user, action: "add_day_off_second" },
      { new: true }
    );
    ...
    bot.sendMessage(
      chatId,
      user.language == "uz"
        ? `<b>2-dam olish kunini </b>tanlang`
        : "Выберите <b>2-й выходной день</b>",
      {
        parse_mode: "HTML",
        reply_markup: {
          remove_keyboard: true,
...
const addDayOffSecond = async (msg) => {
  ...
  if (DaysUz.includes(text) || DaysRu.includes(text)) {
    ...
    await Application.findByIdAndUpdate(application._id, {
      ...application,
      day_off: `${application.day_off},${text}`,
    });
...
const addComment = async (msg) => {
  ...
  if (text == "Ortga qaytish" || text == "Назад") {
    ...
    if (application.time == "Смена") {
      ...
      await Application.findByIdAndUpdate(application._id, {
        ...application,
        day_off: `${application.day_off.split(",")[0]}`,
      });

      let findlanguageDays = user.language == "uz" ? DaysUz : DaysRu;

      let filteredArr = findlanguageDays.filter(
        (e) => e != application.day_off.split(",")[0]
      );
      let sortDaysArr = [];
      filteredArr.map((e, i) =>
        (i + 1) % 2 != 0
...
${
  application.time == "Смена"
    ? `<b>Supervayzer </b> : ${application.supervayzer}`
    : `<b>Dam olish kunlari:</b> ${application?.day_off}`
}
<b>Sababi:</b> ${text}
...
${
  application.time == "Смена"
    ? `<b>Супервайзер </b> : ${application.supervayzer}`
    : `<b>Выходные дни:</b> ${application?.day_off}`
}
<b>Причина:</b> ${text}
...
const sentApplication = async (msg) => {
  ...
  if (text == `Jo‘natish` || text == "Отправить") {
    ...
    bot.sendMessage(
      chatId,
      user.language == "uz"
        ? `Noto'g'ri xabar yubordingiz`
        : `Вы отправили неправильное сообщение`,
      {
        reply_markup: {
          keyboard: [
            [
...
const showApplication = async (msg) => {
  ...
  const findAllApplications = await Application.find({ sent: true })
    .populate(["user"])
    .lean();
...
${
  e.time == "Смена"
    ? `<b>Supervayzer </b> : ${e.supervayzer}`
    : `<b>Dam olish kunlari:</b> ${e?.day_off}`
}
...
${
  e.time == "Смена"
    ? `<b>Супервайзер </b> : ${e.supervayzer}`
    : `<b>Выходные дни:</b> ${e?.day_off}`
}
...
Если вы одобрите заявку оператора "✅"
    ...
    bot.sendMessage(
      chatId,
      user.language == "uz"
        ? `Sorovlar Hozircha yo'q, Menyuni tanlang, ${
            user.admin ? "Admin" : user.full_name
          }`
        : `Выберите меню, ${user.admin ? "Admin" : user.full_name}`,
      {
        reply_markup: {
          keyboard:
            user.admin == true
...
const ApplicationChat = async (query) => {
  ...
  if (!senderUser.admin) {
    if (senderUser.messageCount) {
      bot.sendMessage(
        senderUser.chatId,
        senderUser.language == "uz"
          ? `Siz Adminga Habar yuborib Boldingiz`
          : `Вы отправили сообщение администратору`,
        {
          reply_markup: {
            keyboard:
              senderUser.language == "uz" ? userKeyboardUz : userKeyboardRU,
            resize_keyboard: true,
          },
        }
      );
      ...
      await User.findByIdAndUpdate(
        senderUser._id,
        { ...senderUser, action: `sent_message_to-${getUser.chatId}` },
        { new: true }
      );
      bot.sendMessage(
        chatId,
        senderUser.language == "uz"
          ? `${getUser.full_name} ga Xabar yuborish`
          : `Отправить сообщение на ${getUser.full_name}`,
        {
          reply_markup: {
            keyboard: [
              [
    ...
    await User.findByIdAndUpdate(
      getUser._id,
      { ...getUser, lastMessageAdmin: ``, messageCount: 0 },
      { new: true }
    );
...
const SentMessagetoUser = async (msg) => {
  ...
  if (text == "Chatni yakunlash" || text == "Закончит чат") {
    ...
    bot.sendMessage(
      ...
      senderUser.language == "uz"
        ? `Menyuni tanlang, ${
            senderUser.admin ? "Admin" : senderUser.full_name
          }`
        : `Выберите меню, ${senderUser.admin ? "Admin" : senderUser.full_name}`,
      {
        reply_markup: {
          keyboard:
    ...
    if (senderUser.messageCount) {
      ...
      let getUser = await User.findOne({
        chatId: senderUser.action.split("-")[1],
      })
        .populate()
        .lean();
      await User.findByIdAndUpdate(
        senderUser._id,
        { ...senderUser, messageCount: senderUser.messageCount + 1 },
        { new: true }
      );
...
${
  findApplication.time == "Смена"
    ? `<b>Supervayzer </b> : ${findApplication.supervayzer}`
    : `<b>Dam olish kunlari:</b> ${findApplication?.day_off}`
}
...
${
  findApplication.time == "Смена"
    ? `<b>Супервайзер </b> : ${findApplication.supervayzer}`
    : `<b>Выходные дни:</b> ${findApplication?.day_off}`
}
...
<b>Дата отправки</b> ${formatDate(findApplication?.createdAt)}
    ...
    await User.findByIdAndUpdate(
      getUser._id,
      { ...getUser, lastMessageAdmin: `${getUser.lastMessageAdmin} ${text}` },
      { new: true }
    );
    ...
    bot.sendMessage(
      getUser.chatId,
      getUser.language == "uz"
        ? `${senderUser.full_name} dan sizga habar :  ${text}`
        : `Сообщение от ${senderUser.full_name} : ${text}`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "Adminga Habar yuborish",
                callback_data: `applicationChat_${senderUser?._id}`,
              },
            ],
          ],
...
const answerApplication = async (query) => {
  ...
  const findApplications = await Application.findOne({ _id: dataArr[2] })
    .populate(["user"])
    .lean();
  let user = await User.findOne({ _id: findApplications.user?._id });
  let findAdmin = await User.findOne({ chatId }).lean();
  await Application.findByIdAndUpdate(findApplications._id, {
    ...findApplications,
    sent: false,
  });
...
${
  findApplications.time == "Смена"
    ? `<b>Supervayzer </b> : ${findApplications.supervayzer}`
    : `<b>Dam olish kunlari:</b> ${findApplications?.day_off}`
}
...
${
  findApplications.time == "Смена"
    ? `<b>Супервайзер </b> : ${findApplications.supervayzer}`
    : `<b>Выходные дни:</b> ${findApplications?.day_off}`
}
...
<b>Дата отправки</b> ${formatDate(findApplications?.createdAt)}
                `,
      {
        ...
        reply_markup: {
          remove_keyboard: true,
          inline_keyboard: [
            [{ text: "ko‘rib chiqildi ✅", callback_data: "data_answer" }],
          ],
        },
      }
    );
    ...
    await writeToSheet([
      [
        ...
        user.phone,
        user.sharePhone,
        findApplications.application_latter,
        findApplications.time,
        findApplications.time == "Смена"
          ? findApplications.supervayzer
          : findApplications.day_off,
        formatDate(findApplications.createdAt),
        "HA",
        formatDate(new Date()),
      ],
    ]);
